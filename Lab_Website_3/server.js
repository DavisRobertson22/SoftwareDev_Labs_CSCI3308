...
app.get('/player_info/select_player', function(req, res) {
  // process.stdout.write(util.inspect(req.query.player_choice, {showHidden: false, depth: null}));
  ...
  var query3 = "SELECT COUNT(visitor_name) from football_games WHERE (SELECT id FROM football_players WHERE id = "+player_id+") = ANY (players);"
  ...
  .then(data => {
  	res.render('pages/player_info',{
        my_title: "Player Info",
        players: data[0],
  			this_player: data[1][0],
        this_player_games: data[2][0]
      })
  })
  .catch(error => {
      // display error message in case an error
